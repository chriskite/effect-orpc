...
- c5ca59e: Add Effect `Stream` support to `.effect()` for event iterators / SSE.
...
- c4decf3: Loosen `EffectProcedureHandler` and `EffectMiddlewareHandler` failure channels to `any`.
...
- 3b51d6d: Emit a one-time `console.warn` when a `.useEffect()` middleware runs while no
...
- 6926760: Docs-only patch: republish so the npm-shipped README includes the "Effect
  Schema in `.input()` / `.output()`" section that landed alongside the 0.6.0
  feature. The 0.6.0 tarball was assembled against an unsynced root README and
  shipped without the new docs; no runtime behavior changes.

## 0.6.0

### Minor Changes

- 737a7ad: Auto-convert Effect `Schema` passed to `.input()` / `.output()`.
...
- b7fa762: Add `.useEffect(fn)` for Effect-native middleware.
...
- d2537eb: Fix `Cannot find module '@chriskite/effect-orpc'` for installed consumers.
...
- 2be9d9e: Update the `typescript` peer-dep range to `^6`. TypeScript 6.0.3 type-checks the codebase cleanly with no source-level changes required.
